...
class Easemob {
	 ...
	 * @param $username 用户名
	 */
	public function nickname($options) {
	...
	public function getToken() {
		...
		if ($fp) {
			...
			if ($arr ['expires_in'] < time ()) {
				...
				$result ['expires_in'] = 6*24*3600 + time();
	 ...
	 * CURL Post
	 */
	private function postCurl($url, $option, $header = array("Content-Type:application/json"), $type = 'POST') {
