...
function uploadStart(file) {
	try {
		...
		//$("#container_first").append("<div style='float:left;width:95px;height:75px;margin-right:5px;background-color:#CCC;text-align:center;line-height:75px;'>正在上传</div>");
...
function uploadSuccess(file, serverData) {
	try {
		...
		var jsonobj = jQuery.parseJSON( serverData );
		
		var thumbnail = jsonobj.Thumbnail;
		//var ids = serverData.split(",");
		//var id = ids[0].split(":")[1];
		var id = jsonobj.ID;
		var url=jsonobj.Url;
		//progress.setImage("<img src='"+thumbnail+"' width=95px height=85px><button type='button' class='btn btn-sm default' onclick='doDelete("+id+")'>删除</button>");
		progress.setImage("<div id='image_"+id+"' style='width:200px;float:left;text-align:center;'><img width='200' height='200' src='"+url+"'><br><button type='button' class='btn btn-sm default' onclick='doDelete("+id+")'>删除</button></div>");
		if($("#image_url").val()!=""){
			$("#image_url").val($("#image_url").val()+","+id);
		}else{
			$("#image_url").val(","+id);
			
		}
		if($("#firstImageUrl").val()==""){
			$("#firstImageUrl").val(url);
		}
...
function uploadError(file, errorCode, message) {
	try {
		...
		case SWFUpload.UPLOAD_ERROR.HTTP_ERROR:
			if(message=="422"){progress.setStatus("此图片已" + "发布");}
			else{
