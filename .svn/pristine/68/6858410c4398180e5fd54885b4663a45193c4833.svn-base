...
class OrderController extends Controller
{
    ...
    public function retrieveAction(Request $request)
    {
		...
		$cid = $json->get('category',0);

		$start = $json->get('start',0);
		$num = $json->get('num',10);
		
		if($cid==1){
			$cid_sql = " where a.status!=5 and a.status!=3 and a.status!=4 ";
		}else if($cid==2){
			$cid_sql = " where a.status=5 or a.status=3 or a.status=4 ";
		}else{
			$cid_sql="";
		}
		...
		$sql = "select a.*,b.username from orderlist as a left join user as b on a.user_id = b.id ".$cid_sql." order by a.id desc limit $start,$num";
    ...
    public function retrieveByFilterNumAction(Request $request)
    {
		...
		if($cid==1){
			$cid_sql = " where status!=5 and status!=3 and status!=4 ";
		}else if($cid==2){
			$cid_sql = " where status=5 or status=3 or status=4 ";
		}else{
			$cid_sql="";
		}
		...
		$sql = "select count(*) from orderlist".$cid_sql;
    ...
    public function postBackAction(Request $request){
		...
		$order_status = mysql_result(mysql_query("select status from orderlist where id = $id"),0);
		if($order_status==3){
   ...
   public function changeMyNodifyAction(Request $request){
    ...
    public function infoAction(Request $request)
    {
		...
		if($order_mark==0){
			...
			$sql = "select b.title,a.order_num,a.status_message,a.money,a.id as order_id,a.status,b.content,b.image_url,b.price,b.id,a.creation_date,c.name as category from orderlist as a left join recommended as b on a.publish_id = b.id left join category as c on c.id = b.cid where a.id = $id";
			$result = mysql_query($sql);
			if(!mysql_num_rows($result)){
			...
			$sql = "select b.title,a.order_num,a.status_message,a.money,a.id as order_id,a.status,a.creation_date,b.image_url,b.content from orderlist as a left join article as b on a.publish_id =b.id where a.id = $id";
			$result = mysql_query($sql);
			if(!mysql_num_rows($result)){
				return new JsonResponse('没数据',400);
			}
